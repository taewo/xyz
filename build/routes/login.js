...
function google_noToken(req, res) {
  ...
  ** 수정중.  
  *******************/
		connection.query('insert into xyzlogin (u_id, device, date) values ( (select u_id from xyzuser where token = ' + connection.escape(data['token']) + ' ) , "iphone" , ' + connection.escape(data['date']) + ');', function (err) {
			if (err) {
				console.log(err);
			}
		});
